class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        cars_data = [[pos, sp] for pos, sp in zip(position, speed)]
        stack = []

        for position, speed in sorted(cars_data)[::-1]:
            ideal_time = (target - position) / speed
            if len(stack) == 0 or stack[-1] < ideal_time:
                stack.append(ideal_time)
